...
def run_loop(context: list[dict], goal: str, actions: list, state: dict, config: dict):
    """
    主执行循环。返回一个结果字典。
    :param context: 历史上下文
    :param goal: 用户输入
    ...
    # 从上下文中最新用户消息启用义务，而不仅仅是线程配置。
    # 这覆盖了恢复路径：当挂起的线程用一条新的、表明执行意图的用户消息重新启动时——
    # 线程的原始配置可能并未设置 require_action_attempt。
    # 也要重置持久化状态标志：_obligation_resolved 和
    # _obligation_nudge_count 会通过线程元数据中的 orchestrator_state
    # 从之前的运行中继承，因此之前工具调用中过时的“已解决”状态会静默抑制新的义务。
