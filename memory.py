...
class ActionStep(MemoryStep):
    ...
    action_output: list = field(default_factory=list)
    ...
    def to_messages(self, summary_mode: bool = False) -> list[ChatMessage]:
        ...
        model_output = self.model_output
        if model_output and not summary_mode:
            message.content = [{"type": "text", "text": model_output.split('</think>')[-1].strip()}]

        if self.tool_calls is not None:
            message.tool_calls = [self._get_tool_call(tc) for tc in self.tool_calls]
        ...
        if self.error is not None:
            error_message = (
                "Error:\n"
                + str(self.error)
                + "\nNow let's retry: 注意不要重蹈覆辙！如果已经重试多次，请尝试完全不同的方法。\n"
            )
